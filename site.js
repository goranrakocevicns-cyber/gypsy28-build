...
let allPosts=[];
let commentCounts={};
const SITE_BASE=location.pathname.startsWith('/gypsy28-build/')?'/gypsy28-build/':'/';
function localUrl(path=''){return SITE_BASE+String(path).replace(/^\//,'');}
...
function mediaUrl(src=''){if(!src)return '';if(/^https?:\/\//i.test(src))return src;return localUrl(src);}
...
async function loadPosts(){const box=document.getElementById('posts');if(!box)return;try{const r=await fetch(localUrl('content/posts/index.json')+'?v='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error('Posts HTTP '+r.status);allPosts=await r.json();renderPosts();loadCommentCounts();}catch(e){console.error(e)}}
...
liveRefreshScript.src=localUrl('live-refresh.js');
